...
namespace TransitionalObjects
{
    ...
    public class BaseTransition : MonoBehaviour
    {
        ...
        public void JumpToEnd(bool checkMessage = true)
        {
        ...
        public virtual void TriggerFadeOut()
        {
            ...
            {
                if(state == TransitionState.Waiting || state == TransitionState.Finished || state == TransitionState.LoopFinished)//if finished, then restart. Otherwise continue the animation were this left off
                    currentTime = fadeOutTime;
